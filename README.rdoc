...
E.g. this
  ...
  ruby bin/it multistrap --directory PATH

would execute something like:

  $ multistrap -f /tmp/config_file-12879343294823984
...
E.g. this
  ...
  ruby bin/it multistrap --directory PATH --xyz VAL

would execute something like:

  $ multistrap -f /tmp/config_file-.... --xyz VAL
...
executed with
  ...
  ruby bin/it multistrap --directory /tmp/test --arch foo --xyz --test BLA
...
and be executed with

  multistrap -f /tmp/config_file-12312313 --xyz --test BLA

=== dry-run

If you add a parameter --dry-run, the configuration file is printed to console and the command that would be executed.
